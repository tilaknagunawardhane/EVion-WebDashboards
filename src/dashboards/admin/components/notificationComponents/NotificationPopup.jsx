  ...
  const fetchNotifications = async () => {
    ...
    // console.log("recipient details: ", userId, userModel)
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/notifications/${userId}`, { userModel },
        {
      ...
      if (notification.relatedEntity && notification.relatedEntity.id) {
        switch (notification.type) {
          ...
          case 'station_rejected':
            navigate(`/stations/${notification.relatedEntity.id}`);
            break;
          case 'new_report':
          ...
          {loading ? (
            ...
            notifications.map(notification => (
              <div
                ...
                className={`p-4 border-b cursor-pointer transition-colors ${!notification.isRead ? 'bg-blue-50' : 'hover:bg-gray-50'}`}
                style={{ borderColor: COLORS.border }}
                onClick={() => handleNotificationClick(notification)}
              >
                <div className="flex items-start">
                  <div className={`w-2 h-2 rounded-full mr-2 mt-2 ${!notification.isRead ? 'bg-blue-500' : 'bg-transparent'}`}></div>
